import java.util.*;
public class Array_3_D_Jagged_Eg2 {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);

        System.out.println("enter the outer array size");
        int dime1 = sc.nextInt();
        int[][][] arr = new int[dime1][][];
        int dimen2 = 0;
        int dimen3 = 0;
        for (int i = 0 ; i<dime1 ; i++){
            System.out.println("enter the size of inner array arr:[ "+i+"]");
            dimen2 = sc.nextInt();
            arr[i] = new int[dimen2][];

        }
        for (int i = 0; i<arr.length;i++){
            for (int j = 0;j<arr[i].length;j++){
                System.out.println("enter the 3rd dimension +arr[" +i+"]["+j+"]");
                dimen3 = sc.nextInt();
                arr[i][j] = new int[dimen3];
            }
        }
